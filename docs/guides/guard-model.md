...
- **ONNX Runtime** resolves from the vendored `third_party/onnxruntime-<os>-<arch>-<ver>`
  package (run `scripts/fetch-onnxruntime.sh` if your platform has none).
- **SentencePiece** is provided by the `guard-spm` vcpkg feature, which is **not**
  installed in standard builds. To fully enable the guard tokenizer, reconfigure
  with:
