...
> **对齐来源：** `README_zh.md` Features 段、[架构指南](guides/architecture_zh.md)、`docs/guides/*.md`、[OpenAPI](openapi.yaml)、[OpenAI 兼容矩阵](openai-compat-matrix.md)、[路线图](ROADMAP_zh.md)。
...
| 集群控制平面 | 当 `deployment.mode: cluster` 时集中式配置分发与节点协调（Redis 共享态）。 | 🔵 | `deployment.mode: cluster` | [控制平面](guides/control-plane_zh.md) |
...
| AegisOps 战略愿景 | 由"AI 网关"演进为"AI 治理平台"的能力矩阵与路线图。 | 📐 | — | [AegisOps 愿景](positioning/aegisops-vision_zh.md) · [路线图](ROADMAP_zh.md) |
...
| Redis 集群模式 | 共享缓存 + 限流 + 熔断态；节点间无状态。 | 🔵 | `deployment.mode: cluster` | [控制平面](guides/control-plane_zh.md) |
...
- [路线图](ROADMAP_zh.md) — AegisOps 战略路线图
